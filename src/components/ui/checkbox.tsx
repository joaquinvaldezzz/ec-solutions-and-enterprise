...
const Checkbox = forwardRef<ElementRef<typeof Root>, CheckboxProps>(
  ({ className, size, ...props }, ref) => (
    <Root
      className={cn(
        'peer aria-invalid:border-error-300 aria-invalid:ring-error-500/25 data-checked:border-brand-600 data-checked:bg-brand-600 shrink-0 cursor-pointer rounded-sm border border-gray-300 bg-white focus:ring-4 focus:ring-gray-400/15 focus:outline-hidden disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-300 data-checked:text-white',
        size === 'sm' ? 'size-4 rounded-sm' : 'size-5 rounded-md',
        className,
      )}
      ref={ref}
      {...props}
